...
export default function CampaignEdit() {
    ...
    const handleAddOperador = () => {
        ...
        if (isEditing && editingParticipant) {
            const metaValorNumber = String(meta_valor);

            console.log(typeof(metaValorNumber))
            console.log(metaValorNumber)

            console.log(typeof(premiacao))
            ...
            const updatedParticipant: Partial<IParticipants> = {
                ...
                meta_valor: tipoMeta === 'VALOR' ? parseFloat(metaValorNumber.replace(/\s/g, '').replace('R$', '').replace(/\./g, '').replace(',', '.')) : 0,
                meta_quantidade: tipoMeta === 'QUANTIDADE' ? parseFloat(metaValorNumber.replace(/\s/g, '').replace('R$', '').replace(/\./g, '').replace(',', '.')) : 0,
                premiacao: String(premiacao).replace(/\s/g, '').replace('R$', '').replace(/\./g, '').replace(',', '.'),
                tipo_meta: tipoMeta,

...
export default function NegotiationsRegistration() {
    ...
    const [tables, setTables] = useState<{ id: number; descricao: string; id_item_objeto?: number; id_item_negociacao?: number }[]>([]);
    const [nextTableId, setNextTableId] = useState(1);
    const [contatosTemp, setContatosTemp] = useState<IContatoTemp[]>([]);
    const [contatoInput, setContatoInput] = useState({ nome: '', email: '', telefone: '', });
    ...
    const [selectedItem, setSelectedItem] = useState<ICadastroItensObjeto | null>(null);
    const [selectedLoja, setSelectedLoja] = useState<ILojaSelect | null>(null);
    const [selectedProduto, setSelectedProduto] = useState<IProdutoSelect | null>(null);
    const [produtoInput, setProdutoInput] = useState({ unidades: 0, valor: 0 });
    ...
    const { error, empresas = [], produtos = [], itensObjeto = [], filiais = [] } = useSelector((state: RootState) => state.tradeNegotiations);
    ...
    const handleSubmit = async () => {
        ...
        try {
            ...
            if (isEditMode) {
                ...
                console.log("Negociação Detalhes")
                console.log(negociationData)
            ...
            console.log(tabelasAtualizadas)

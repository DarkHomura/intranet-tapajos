...
export interface INegotiationState {
    ...
    itensObjetoAtual: ICadastroItensObjeto | null
    loading: boolean;
    error: string | null;
...
export const fetchNegotiationCampaigns = createAsyncThunk(
    ...
    async (_, { rejectWithValue }) => {
        try {
            ...
            const response = await apiInstance.get('varejo/NegociacaoVarejo/')
            const dataFilter = response.data.filter((item:any) => item.status == true)
            return dataFilter;
        } catch (error) {
...
export const fetchNegotiationCampaignsByStatus = createAsyncThunk(
    'tradeNegotiations/fetchCampaignsByStatus',
    async (status:boolean, { rejectWithValue }) => {
        try {
            const response = await apiInstance.get('varejo/NegociacaoVarejo/');
            const dataFilter = response.data.filter((item:any) => item.status == status)
            return dataFilter;
        } catch (error) {
            if (axios.isAxiosError(error)) {
                return rejectWithValue(error.response?.data || error.message);
...
export const updateItemObjeto = createAsyncThunk (
    'tradeNegotiations/updateItemObjeto',
    async (
        {id, data,} : {id:number; data:Partial<ICadastroItensObjeto>}, {rejectWithValue} 
    ) => {
        try{
            const response = await apiInstance.put(`varejo/NegociacaoVarejoObjeto/${id}`, data)
            return response.data
        }catch(error) {
            if(axios.isAxiosError(error)){
                return rejectWithValue(error.response?.data || error.message)
...
export const deleteItemObjeto = createAsyncThunk(
    'tradeNegotiations/deleteItemObjeto', async(id:number, {rejectWithValue}) =>{
        try{
            await apiInstance.delete(`varejo/NegociacaoVarejoObjeto/${id}`)
            return id
        }catch(error) {
            if (axios.isAxiosError(error)){
                return rejectWithValue(error.response?.data || error.message)
...
//Alteração
        setItensObjetoAtual: (
            state,
            action: PayloadAction<ICadastroItensObjeto | null>
        ) => {
            state.itensObjetoAtual = action.payload;
        },
...
//DeleteItemObjeto
            .addCase(deleteItemObjeto.pending, (state) =>{
                state.loading = true
            })
            .addCase(deleteItemObjeto.fulfilled, (state, action) => {
                state.loading = false
                state.itensObjeto = state.itensObjeto.filter((item) => item.id !== action.payload)
                if(state.itensObjetoAtual?.id === action.payload){
                    state.itensObjetoAtual = null
                }
            })
            .addCase(deleteItemObjeto.rejected, (state, action) => {
                state.loading = false
                state.error = action.payload as string
            })

//updateItemObjeto
            .addCase(updateItemObjeto.pending, (state) => {
                state.loading = true
            })

            .addCase(updateItemObjeto.fulfilled, (state, action) =>{
               state.loading = false
               const index = state.itensObjeto.findIndex(
                (item) => item.id === action.payload.id
               )
               if (index !== 1){
                state.itensObjeto[index] = action.payload
               }
               if(state.itensObjetoAtual?.id === action.payload){
                state.itensObjetoAtual = action.payload
               }
            ...
            //Alteração
            .addCase(fetchNegotiationCampaignsByStatus.fulfilled, (state, action) =>{
                state.campaigns = action.payload;
            })
